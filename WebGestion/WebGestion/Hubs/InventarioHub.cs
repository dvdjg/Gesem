...
namespace WebGestion.Hubs
{
    ...
    using RecordBasic = Models.InventarioBase;
    using Record = Models.InventarioVM;
    using RecordModel = Models.Inventario;
    using RecordSet = DbSet<Models.Inventario>;
    using Context = Models.GesemEntities;
    using Microsoft.AspNet.SignalR;
    using System.Collections.Generic;
    using Models;

    [HubName("InventarioHub")]
    public class InventarioHub : RecordsHub<ModelRecord, RecordBasic, Record, RecordModel, RecordSet, Context>
    {
        ...
        protected override IQueryable<RecordModel> Filter(Context context, string search, int Id, string columnFilters_0, string columnFilters_1, string columnFilters_2, string columnFilters_3, string columnFilters_4, string columnFilters_5, string columnFilters_6, string columnFilters_7)
        {
            var results = getRecordSet(context).Where(p => // p.Id != 0 && p.EstadoId != 12 &&  // 12 == Borrado
                (search == null || (
                p.CodUnico.StartsWith(search)
            || p.Bienes.Descripcion.StartsWith(search)))
                && (columnFilters_0 == null || p.Id.Equals(Id))
                && (columnFilters_1 == null || (p.CodUnico.StartsWith(columnFilters_1)))
                && (columnFilters_2 == null || (p.Bienes.Descripcion != null && p.Bienes.Descripcion.StartsWith(columnFilters_2)))
                );
            return results;
        }
        ...
        {
            FindValidRegister(context, record, "CodUnico");
        }
        //////////////////////////////////////////////////////////////////////////////////////////////////
        //
        // Métodos públicos
