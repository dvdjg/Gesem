...
namespace WebGestion.Hubs
{
    ...
    using RecordBasic = Models.EmpleadosBase;
    using Record = Models.EmpleadosVM;
    using RecordModel = Models.Empleados;
    using RecordSet = DbSet<Models.Empleados>;
    using Context = Models.GesemEntities;
    using Microsoft.AspNet.SignalR;
    using Models;

    [HubName("EmpleadosHub")]
    public class EmpleadosHub : RecordsHub<ModelRecord, RecordBasic, Record, RecordModel, RecordSet, Context>
    {
        ...
        protected override IQueryable<RecordModel> Filter(Context context, string search, int Id, string columnFilters_0, string columnFilters_1, string columnFilters_2, string columnFilters_3, string columnFilters_4, string columnFilters_5, string columnFilters_6, string columnFilters_7)
        {
            var results = getRecordSet(context).Where(p => // p.Id != 0 && p.EstadoId != 12 &&  // 12 == Borrado
               (search == null || (
                p.CodEmpleado.StartsWith(search)
             || p.Lugar.StartsWith(search)))
                && (columnFilters_0 == null || p.Id.Equals(Id))
                && (columnFilters_1 == null || p.CodEmpleado.StartsWith(columnFilters_1))
                && (columnFilters_2 == null || p.Lugar.StartsWith(columnFilters_2))
                );
            return results;
        }
        ...
        {
            FindValidRegister(context, record, "CodEmpleado");
        }
        //////////////////////////////////////////////////////////////////////////////////////////////////
        //
        // Métodos públicos
