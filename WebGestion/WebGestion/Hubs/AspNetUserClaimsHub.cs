...
namespace WebGestion.Hubs
{
    ...
    using RecordBasic = Models.AspNetUserClaimsBase;
    using Record = Models.AspNetUserClaimsVM;
    using RecordModel = Models.AspNetUserClaims;
    using RecordSet = DbSet<Models.AspNetUserClaims>;
    using Context = Models.GesemEntities;
    using Microsoft.AspNet.SignalR;
    using Models;

    [HubName("AspNetUserClaimsHub")]
    public class AspNetUserClaimsHub : RecordsHub<ModelRecord, RecordBasic, Record, RecordModel, RecordSet, Context>
    {
        ...
        protected override IQueryable<RecordModel> Filter(Context context, string search, int Id, string columnFilters_0, string columnFilters_1, string columnFilters_2, string columnFilters_3, string columnFilters_4, string columnFilters_5, string columnFilters_6, string columnFilters_7)
        {
            var results = getRecordSet(context).Where(p => // p.Id != 0 && p.EstadoId != 12 &&  // 12 == Borrado
               (search == null || (
                p.ClaimType.StartsWith(search)
             || p.ClaimValue.StartsWith(search)))
                && (columnFilters_0 == null || p.Id.Equals(Id))
                && (columnFilters_1 == null || p.ClaimType.StartsWith(columnFilters_1))
                && (columnFilters_2 == null || p.ClaimValue.StartsWith(columnFilters_2))
                );
            return results;
        }
        //////////////////////////////////////////////////////////////////////////////////////////////////
